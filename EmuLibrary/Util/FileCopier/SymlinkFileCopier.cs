...
namespace EmuLibrary.Util.FileCopier
{
    public class SymlinkFileCopier : BaseFileCopier, IFileCopier
    {
        ...
        private static void CreateDirectorySymlinks(DirectoryInfo source, DirectoryInfo destination)
        {
        ...
        private static void CreateFileSymlink(FileInfo sourceFile, DirectoryInfo destination)
        {
            ...
            CreateLinkWithFallback(linkPath, sourceFile.FullName);
        }

        private static void CreateLinkWithFallback(string linkPath, string targetPath)
        {
            ...
            if (AreOnSameVolume(linkPath, targetPath) && TryCreateMklink($"/c mklink /H \"{linkPath}\" \"{targetPath}\"", out _))
            {
                return;
            }
            ...
            {
                throw new Exception($"Failed to create link from \"{linkPath}\" to \"{targetPath}\". Symlink error: {symlinkError}. Copy fallback error: {copyEx.Message}", copyEx);
            }
        }
