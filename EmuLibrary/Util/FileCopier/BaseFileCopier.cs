...
namespace EmuLibrary.Util.FileCopier
{
    public abstract class BaseFileCopier : IFileCopier
    {
        ...
        public async Task CopyAsync(CancellationToken cancellationToken)
        {
            ...
            {
                throw new FileCopierValidationException($"source path \"{Source.FullName}\" does not exist or is invalid");
            }
            if (Destination.FullName.IsNullOrEmpty())
            {
                throw new FileCopierValidationException($"destination path \"{Destination.FullName}\" is not valid");
            }
